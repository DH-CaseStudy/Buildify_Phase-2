...
### 2. 🔐 사용자 로그인
- Spring Security를 활용하여 사용자 인증(Authentication) 및 인가(Authorization) 기능
...
### 3. 사용자 창고 등록
- 지역별 창고의 이미 사용중인 섹션 표시
- 창고 데이터베이스의 주소값을 기반으로 카카오 지도 api 연동
- 원하는 지역의 창고의 섹션을 고르고 기간 별 사용 등록할 수 있는 기능


### 4. 등록된 창고 조회
- 유저가 사용중인 창고의 현황을 조회할 수 있는 기능

### 4. 📦 사용자 상품 등록
- 상품 등록 시, 대분류/중분류/소분류에서 고른 카테고리 값을 기반으로 카테고리 테이블에서 카테고리ID 산출
- 카테고리ID와 함께 상품 정보 등록 기능

### 6. 🔍 등록된 상품 조회
- 유저가 등록한 상품을 조회할 수 있는 기능

### 7. 🔍 유저 대시보드 화면
- 일간/주간 입고/출고 요청과 승인의 건수 표시
- 주간 입고/출고 요청과 승인의 건수 추이 차트
- 물류뉴스 API/ 창고의 지역 별 날씨 API 연동
